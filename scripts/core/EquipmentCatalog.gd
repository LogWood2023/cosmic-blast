...
const AUXILIARIES: Dictionary = {
	...
	"divine_drone_seed": {
		...
		"mineral_bonus": 0.0,
		"drone_slots": 1,
		"drone_fire_interval_mult": 0.92,
		"drone_damage_mult": 1.12,
	...
	"divine_oracle_swarm_core": {
		...
		"mineral_bonus": 0.08,
		"drone_slots": 3,
		"drone_fire_interval_mult": 0.68,
		"drone_damage_mult": 1.45,
...
const AUXILIARY_EXPANSION_ROWS: Array[Dictionary] = [
	...
	{"id": "warped_quarry_lens", "name": "扭曲采场透镜", "family": FAMILY_WARPED, "rarity": "rare", "price": 158, "compute_cost": 5, "description": "透镜折弯矿尘的质量影，松散碎矿会被拖回弹道中心。", "stats": {"mineral_bonus": 0.12, "gravity_pull_strength": 680.0, "gravity_pull_radius": 230.0}},
...
static func make_player_stats(weapon_id: String, aux_ids: Array[String]) -> Dictionary:
	...
	for aux_id in aux_ids:
		...
		stats["bullet_split_damage_mult"] = maxf(float(stats["bullet_split_damage_mult"]), float(aux.get("bullet_split_damage_mult", 0.0)))
		...
		stats["dash_aftershock_radius"] = maxf(float(stats["dash_aftershock_radius"]), float(aux.get("dash_aftershock_radius", 0.0)))
		stats["dash_aftershock_damage_mult"] = maxf(float(stats["dash_aftershock_damage_mult"]), float(aux.get("dash_aftershock_damage_mult", 0.0)))
		stats["bullet_speed_mult"] = float(stats["bullet_speed_mult"]) * float(aux.get("bullet_speed_mult", 1.0))
		stats["homing_strength"] = maxf(float(stats["homing_strength"]), float(aux.get("homing_strength", 0.0)))
		stats["homing_range"] = maxf(float(stats["homing_range"]), float(aux.get("homing_range", 0.0)))
		stats["gravity_pull_strength"] = maxf(float(stats["gravity_pull_strength"]), float(aux.get("gravity_pull_strength", 0.0)))
