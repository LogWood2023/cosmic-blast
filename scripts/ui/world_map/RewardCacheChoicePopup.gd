...
func setup(node: Dictionary, choices: Array) -> void:
	title_label.text = "奖励缓存选择"
	meta_label.text = "阶层%d / %s / %s" % [
		int(node.get("tier", 1)),
		String(node.get("reward_title", "未标记缓存")),
		_family_text(String(node.get("cache_family_bias", node.get("family_bias", "")))),
	]
...
func _add_choice_button(choice: Dictionary) -> void:
	...
	var lines: Array[String] = [
		String(choice.get("title", "未知缓存")),
		String(choice.get("description", "缓存内容仍在解析。")),
		String(choice.get("preview", "方舟会按当前航路处理缓存。")),
	]
	button.text = "\n".join(lines)
