...
func _run() -> void:
	...
	room.setup_room_config({
		...
		"trap_count": 4,
		"chest_crystal_count": 4,
		"clutter_count": 24,
		"colossus_family_weight": 1.0,
		"paradise_family_weight": 0.0,
		"warped_family_weight": 0.0,
		"hell_eye_family_weight": 0.0,
		"divine_family_weight": 0.0,
	...
	if GameManager.stutter_context != READY_CONTEXT:
		_fail("ExploreRoom did not finish loading before cleanup timeout. Last context: %s" % GameManager.stutter_context)
		return
	remove_child(room)
	room.queue_free()
	await _finish_explore_room_cleanup(room)
	print("Explore room cleanup check passed.")
	get_tree().quit(0)
