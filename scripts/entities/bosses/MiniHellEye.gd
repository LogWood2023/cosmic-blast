...
const HealthBarScript = preload("res://scripts/ui/HealthBar.gd")
...
static func spawn(parent: Node, pos: Vector2, sc: float, hp_val: int) -> MiniHellEye:
	...
	var clip_shader = parent._nebula_mat.shader if is_instance_valid(parent._nebula_mat) else preload("res://assets/shaders/helleye/eye_clip.gdshader")
